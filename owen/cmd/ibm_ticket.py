...
class TicketShell(cli.Shell):
    ...
    def do_ticket_list(self, args):
        ...
        if not args.closed:
            q = q.filter(models.PartRequest.closed != True)
    ...
    def do_part_list(self, args):
        self._table(self.session.query(models.Part).\
                                 order_by(models.Part.id),
                    ['id', 'fru', 'description'])
    ...
    def do_part_alter(self, args):
        q = self.session.query(models.Part).\
                         filter(models.Part.id == args.part)
        part = q.first()
        if not part:
    ...
    def do_host_list(self, args):
        self._table(self.session.query(models.Machine).\
                                 order_by(models.Machine.id),
